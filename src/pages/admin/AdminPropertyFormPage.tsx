...
import { Building2, ArrowLeft, Upload, Trash2, CheckCircle2, Image as ImageIcon, AlertTriangle, X, Star } from 'lucide-react';
...
export const AdminPropertyFormPage: React.FC<AdminPropertyFormPageProps> = ({ propertyId, onNavigate }) => {
  ...
  const handleFileUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      const uploadPromises = files.map(file => propertyService.uploadImage(file));
      ...
      setFormData(prev => {
        ...
        return {
          ...prev,
          main_image: prev.main_image || newImages[0] || '',
          images: newImages
        };
      });
    } catch (err) {
      ...
      <form onSubmit={handleSubmit} className="space-y-8 bg-white p-8 sm:p-10 rounded-3xl border border-gray-100 shadow-xl">
        ...
        <div className="space-y-4 pb-6 border-b border-gray-100">
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            {/* File Upload Button */}
            <label className="w-full sm:w-auto bg-[#12372A] hover:bg-[#0d281e] text-white font-bold text-xs uppercase tracking-wider py-3.5 px-5 rounded-xl shadow cursor-pointer flex items-center justify-center space-x-2 transition-all">
              <Upload className="w-4 h-4 text-[#D4AF37]" />
              <span>{uploadingImage ? 'Téléchargement...' : 'Sélectionner plusieurs photos'}</span>
              <input
                type="file"
                accept="image/*"
                multiple
            ...
            <div className="flex-1 flex gap-2 w-full sm:w-auto">
              ...
              <button
                ...
                className="bg-gray-100 hover:bg-gray-200 text-gray-800 font-bold px-4 py-2 rounded-xl text-xs transition-colors"
              >
                Ajouter
              </button>
            </div>
          ...
          {formData.images && formData.images.length > 0 && (
            <div className="space-y-2 pt-3">
              <p className="text-xs font-semibold text-gray-500">
                {formData.images.length} photo(s) ajoutée(s). Cliquez sur <strong className="text-[#12372A]">⭐ Image de Couverture</strong> pour choisir la photo principale.
              </p>
              <div className="grid grid-cols-2 sm:grid-cols-4 gap-4">
                {formData.images.map((imgUrl, idx) => {
                  const isMain = formData.main_image === imgUrl;
                  return (
                    <div key={idx} className={`relative group rounded-2xl overflow-hidden aspect-[4/3] bg-gray-100 border-2 transition-all ${
                      isMain ? 'border-[#D4AF37] shadow-md ring-2 ring-[#D4AF37]/30' : 'border-gray-200 hover:border-gray-300'
                    }`}>
                      <img src={imgUrl} alt="" className="w-full h-full object-cover" />
                      
                      {/* Main / Cover Image Badge or Action */}
                      {isMain ? (
                        <div className="absolute top-2 left-2 bg-[#12372A] text-[#D4AF37] text-[10px] font-extrabold px-2.5 py-1 rounded-lg shadow flex items-center space-x-1 z-10">
                        ...
                        <button
                          ...
                          className="absolute top-2 left-2 bg-black/75 hover:bg-[#12372A] text-white text-[10px] font-bold px-2.5 py-1 rounded-lg shadow opacity-90 sm:opacity-0 group-hover:opacity-100 transition-all flex items-center space-x-1 z-10"
                        >
                      ...
                      <button
                        ...
                        className="absolute top-2 right-2 p-1.5 rounded-lg bg-red-600 hover:bg-red-700 text-white shadow opacity-90 sm:opacity-0 group-hover:opacity-100 transition-all z-10"
